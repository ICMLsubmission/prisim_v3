...
def numericalBinning(original_data, feature_numerical_noise_bin):
    ...
    for feature in feature_numerical_noise_bin.keys():
        ...
        if len(auto_bin) > 0:
            ...
            smart_bin = {}
            c = 0
            grouped_mean = data.groupby("noisy_feature_binned").mean(numeric_only=True)
            for i in grouped_mean.index:
                smart_bin[i] = grouped_mean.noisy_feature.values[c]
                c += 1
...
def date_zip_noise_binning(data, features_noise_bin):
    for i in features_noise_bin.keys():
        if features_noise_bin[i]["noise"] != 0 :
            ...
            data[i] += pd.DateOffset(noise_value)
...
def hybrid_dp(
    ...
    # get static bin intervals from original data
...
def general_quality_index(data_o, data_g):
    ...
    df_R = corr_R.where(np.triu(np.ones(corr_R.shape)).astype(np.bool_))
    df_R = df_R.stack().reset_index()
    df_R.columns = ["Feature1_R", "Feature2_R", "Corr_R"]

    df_G = corr_G.where(np.triu(np.ones(corr_G.shape)).astype(np.bool_))
